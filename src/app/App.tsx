...
import { themeIdsForEntry, type CanonicalThemeId } from '../content/taxonomy'
  ...
  if (screen === 'home') {
    ...
    return (
      <main className="shell">{updateBanner}
        ...
        <fieldset className="direction-picker"><legend><FormattedMessage id="direction" /></legend><div className="direction-switch">{activeLanguagePair.directions.map((config) => <button key={config.id} type="button" aria-pressed={settings.direction === config.id} onClick={() => void changeDirection(config.id)}><FormattedMessage id={config.displayMessageId} /></button>)}</div><p className="helper"><FormattedMessage id="directionHelper" /></p></fieldset>
