...
import { activeLanguagePair } from '../i18n/languagePairs'
...
export async function ensureCatalogSchedules(database: ReversolinguoDatabase = db, now = new Date()): Promise<number> {
  ...
  const missing = catalog.flatMap((entry) => activeLanguagePair.directions.map((direction) => initialSchedule(entry.id, direction.id, now)))
