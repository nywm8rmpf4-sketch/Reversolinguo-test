...
function fail(message) { throw new Error(`A1_PROMOTION_FAIL: ${message}`) }
...
function stableSemanticProjection(entry) {
  const projection = { entry_id: entry.entry_id, language_tag: entry.language_tag, lemma: entry.lemma, part_of_speech: entry.part_of_speech, senses: entry.senses, cefr_level: entry.cefr_level, themes: entry.themes, variety: entry.variety }
  if ('gender' in entry) projection.gender = entry.gender
  if ('article' in entry) projection.article = entry.article
  return projection
}

const partFiles = fs.readdirSync(DRAFTS).map((name) => {
...
for (const entryId of overlapIds) {
  ...
  if (JSON.stringify(stableSemanticProjection(current)) !== JSON.stringify(stableSemanticProjection(reviewedDraft))) overlapMismatches.push(entryId)
}
...
const newEntries = staging.filter((entry) => !baseIds.has(entry.entry_id)).map((entry) => ({ ...entry, provenance: { ...entry.provenance, reviewed_by: REVIEWED_BY, reviewed_at: REVIEWED_AT }, status: 'reviewed' }))
...
const manifest = { ...previousManifest, catalog_version: CATALOG_VERSION, entry_count: EXPECTED_FINAL_COUNT, catalog_sha256: catalogSha256, status: 'validated', human_review: 'PASS' }
...
const signatureDocument = { key_id: KEY_ID, algorithm: 'ECDSA-P256-SHA256', signature_base64: signature.toString('base64') }
...
const verified = (await import('node:crypto')).verify('sha256', Buffer.from(manifestText, 'utf8'), { key: publicKey, dsaEncoding: 'ieee-p1363' }, signature)
...
const report = { generated_at: new Date().toISOString(), source_staging: { expected_candidate_sha: '3076a602351ce9bc21b600f8119ae5b704f77f10', draft_count: staging.length, tranche_part_files: partFiles.map(({ name }) => name) }, base_count: base.length, historical_count: EXPECTED_HISTORICAL_COUNT, historical_json_sha256: historicalHash, overlap_count: overlapIds.length, overlap_semantic_mismatches: overlapMismatches, new_count: newEntries.length, final_count: promoted.length, final_unique_count: promotedIds.size, catalog_sha256: catalogSha256, catalog_version: CATALOG_VERSION, key_id: KEY_ID, private_key_persisted: false, human_review: 'PASS', reviewed_by: REVIEWED_BY, reviewed_at: REVIEWED_AT }
