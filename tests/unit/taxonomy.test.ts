...
describe('canonical thematic taxonomy', () => {
  ...
  it('covers exactly every active v1.0.1 lexical UUID with known non-empty themes', () => {
    expect(activeEntryIds.size).toBe(24)
    expect(v1_0_1ThemeAssignments).toHaveLength(24)
    expect(validateTaxonomyAssignments(v1_0_1ThemeAssignments, activeEntryIds)).toEqual({ valid: true, errors: [] })
    expect(new Set(v1_0_1ThemeAssignments.map((assignment) => assignment.entry_id))).toEqual(activeEntryIds)
  })
