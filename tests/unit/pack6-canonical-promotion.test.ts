...
const activeB2 = activeCanonical.filter((entry) => entry.cefr_level === 'B2')

describe('cumulative A1-B2 canonical runtime promotion invariants', () => {
  it('derives runtime volume from the current canonical data instead of a hard-coded corpus size', () => {
    ...
    const b2 = pack6BAdultPacks.find((pack) => pack.pack_id === adultPackId('B2'))
    ...
    expect(b2?.entries).toHaveLength(activeB2.length)
    ...
    expect(new Set(b2?.entries.map((entry) => entry.entry_id))).toEqual(new Set(activeB2.map((entry) => entry.entry_id)))
    ...
    expect(resolveLearningPack(adultPackId('B2'), pack6BAdultPacks)).toHaveLength(new Set([...activeA1, ...activeA2, ...activeB1, ...activeB2].map((entry) => entry.entry_id)).size)
  })
