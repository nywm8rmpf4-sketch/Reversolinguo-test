...
test('A - editorial drafts select targeted QA and reject runtime over-test', () => {
  const result = classify([
    'catalogs/fr-es/a1/drafts/a1-tranche3.json',
    'tests/unit/a1-tranche3-staging.test.ts',
    'evidence/active/A1_TRANCHE3.md'
  ]);
...
test('mixed editorial and runtime diff escalates to runtime controls', () => {
  const result = classify(['catalogs/fr-es/a1/drafts/a1-tranche3.json', 'catalogs/fr-es/a1/catalog.json']);
  assert.equal(result.validation_profile, 'RUNTIME_CONTENT');
  assert.equal(result.transversal, true);
  assert.ok(result.required_controls.includes('LEXICAL_TARGETED'));
...
test('PASS reuse proof is fail-closed and requires applicability fingerprints', () => {
  ...
  assert.doesNotThrow(() => validateReuseProof({
    evidence_ref: 'evidence/active/EXAMPLE.md',
    source_sha: 'a'.repeat(40),
    target_sha: 'b'.repeat(40),
    contract_fingerprint: 'contract-v1',
    data_fingerprint: 'data-sha256:example',
    environment_fingerprint: 'node24-ubuntu',
    rationale: 'unchanged component and gate'
  }, policy));
});

test('public workflow contract keeps heavy QA away from staging and guards candidate first', { skip: !existsSync('.github/workflows/qa.yml') }, () => {
  ...
  assert.ok(runtime.indexOf('qa-governance.mjs plan') < runtime.indexOf('npm ci'));
