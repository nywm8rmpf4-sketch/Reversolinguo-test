...
test('FR-EN temporary pair switches outside session, learns and survives offline reload', async ({ page, context, browserName }) => {
  ...
  await page.getByRole('button', { name: 'Vocabulaire' }).click()
  await expect(page.getByRole('button', { name: 'Français (fr-FR) → Anglais (en-GB)' })).toBeVisible()
  await expect(page.getByRole('button', { name: 'Anglais (en-GB) → Français (fr-FR)' })).toBeVisible()
  await expect(page.getByRole('button', { name: /Espagnol d’Espagne/ })).toHaveCount(0)
  ...
  expect(frEsBackground).toContain('.webp')
